...
EvalResults = Dict[str, Dict[str, List[float]]]
...
class Scoring:
    ...
    def load_ingestion_results(self, eval_path: str, score_dir: str) -> None:
        """
        ...
        logger.info(f"Read ingestion results from {eval_path}")
    ...
    def compute_bootstrapped_scores(self, eps=1e-3, n_bootstraps=10, sample_size=50, random_seed=42):
        ...
        self.bootstrapped_scores = {
            "interval": intervals,
            "coverage": coverages,
            "quantiles_score": quantiles_scores,
        }

    def plot_bootstrapped_scores(self):
        self._print_center("\n=====================================\n")
        ...
        self._print_center("\n=====================================\n")
        self._print_center("Bootstrapped Scores")
        self._print_center("\n=====================================\n")

        fig, axs = plt.subplots(1, 3, figsize=(18, 6))

        # Plot histogram of bootstrapped quantiles scores
        std_err_quantiles = np.array(self.bootstrapped_scores["quantiles_score"]).std()
        axs[0].hist(
            self.bootstrapped_scores["quantiles_score"],
            bins=20,
            color="r",
            alpha=0.7,
            label=f"Bootstrapped Scores {self.name} (std_err={std_err_quantiles:.3f})",
        )
        axs[0].set_xlabel("Score")
        axs[0].set_ylabel("Frequency")
        axs[0].set_title("Histogram of Bootstrapped Scores")
        axs[0].legend()
        axs[0].grid(True)

        # Plot histogram of bootstrapped coverage
        std_err_coverage = np.array(self.bootstrapped_scores["coverage"]).std()
        axs[1].hist(
            self.bootstrapped_scores["coverage"],
            bins=20,
            color="g",
            alpha=0.7,
            label=f"Bootstrapped Coverage {self.name} (std_err={std_err_coverage:.3f})",
        )
        axs[1].set_xlabel("Coverage")
        axs[1].set_ylabel("Frequency")
        axs[1].set_title("Histogram of Bootstrapped Coverage")
        axs[1].legend()
        axs[1].grid(True)

        # Plot histogram of bootstrapped interval lengths
        std_err_interval = np.array(self.bootstrapped_scores["interval"]).std()
        axs[2].hist(
            self.bootstrapped_scores["interval"],
            bins=20,
            alpha=0.7,
            color="b",
            label=f"Bootstrapped Interval Length {self.name} (std_err={std_err_interval:.3f})",
        )
        axs[2].set_xlabel("Interval Length")
        axs[2].set_ylabel("Frequency")
        axs[2].set_title("Histogram of Bootstrapped Interval Length")
        axs[2].legend()
        axs[2].grid(True)
    ...
    def write_scores(self):
        ...
        os.makedirs(Path(self.score_file).parent, exist_ok=True)
